...
const PhoneInput: React.ForwardRefExoticComponent<PhoneInputProps> = React.forwardRef<
  ...
  PhoneInputProps
>(({ className, onChange, value, size = "default", ...props }, ref) => {
  return (
    <RPNInput.default
      ref={ref}
      className={cn("flex", className)}
      flagComponent={FlagComponent}
      countrySelectComponent={({ size, ...props }) => <CountrySelect size={size} {...props} />}
    ...
    >
      <PopoverTrigger asChild>
        <Button
          type="button"
          variant="outline"
          className={`flex gap-1 rounded-s-lg rounded-e-none border-r-0 focus:z-10 ${inputVariants.size[size]}`}
          disabled={disabled}
        >
